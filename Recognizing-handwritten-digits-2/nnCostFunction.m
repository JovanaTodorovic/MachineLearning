...
z3=a2*Theta2';
a3=sigmoid(z3);
J=-(sum(sum(yp.*log(a3)+(1-yp).*log(1-a3))))/m;
br=sum(sum(Theta1(:,2:end).^2))+sum(sum(Theta2(:,2:end).^2));
J=J+lambda*br/(2*m);
...
%for i=1:m
%   delta3= a3(i,:)-yp(i,:);
%  % delta2(i,:)=delta3(i,:)*Theta2(:,2:end);
 %  delta2=delta3*Theta2(:,2:end);
%   delta2=delta2.*sigmoidGradient(z2(i,:));
%  % delta1(i,:)=delta2(i,:)*Theta1(i,2:end).*sigmoidGradient(z2(2:end));
 %  Delta3=Delta3+(delta3)'*a2(i,:);
 %  Delta2=Delta2+(delta2)'*X(i,:);
%end;
